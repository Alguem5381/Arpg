...
public class SheetController(SheetServices sheetServices) : BaseController
{
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] ComputeSheetDto request)
    {
        if (id != request.Id)
            return ToFailResults(Result.Fail(new ValidationError("Sheet id mismatch.")
                .WithMetadata(MetadataKey.Error, SheetCodes.SheetMismatch)));
