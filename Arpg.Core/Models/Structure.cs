...
public class TemplateStructure
{
    ...
    public Result Update(BatchUpdate batch)
    {
        ...
        foreach (var catOp in batch.Categories ?? [])
        {
            var result = ProcessCategory(this, catOp);
            if (result.IsFailed)
                errors.AddRange(result.Errors);
        }

        foreach (var fieldOp in batch.Fields ?? [])
        {
            var result = ProcessField(this, fieldOp);
            if (result.IsFailed)
                errors.AddRange(result.Errors);
        }
        ...
        errors.AddRange(Fields
            ...
            .Select(f => new InvalidFieldError
                (
                    $"Field '{f.Name}' references a missing Category ({f.CategoryId}).",
                    f.Id,
                    f.CategoryId
                )
    ...
    private static Result ProcessCategory(TemplateStructure structure, CategoryOperation op)
    {
        return op.Op switch
        {
    ...
    private static Result AddCategory(TemplateStructure structure, CategoryOperation op)
    {
    ...
    private static Result EditCategory(TemplateStructure structure, CategoryOperation op)
    {
    ...
    private static Result DeleteCategory(TemplateStructure structure, CategoryOperation op)
    {
        var cat = structure.Categories.FirstOrDefault(c => c.Id == op.Id);
    ...
    private static Result ProcessField(TemplateStructure structure, FieldOperation op)
    {
        return op.Op switch
        {
    ...
    private static Result AddField(TemplateStructure structure, FieldOperation op)
    {
    ...
    private static Result EditField(TemplateStructure structure, FieldOperation op)
    {
    ...
    private static Result DeleteField(TemplateStructure structure, FieldOperation op)
    {
        var field = structure.Fields.FirstOrDefault(f => f.Id == op.Id);
